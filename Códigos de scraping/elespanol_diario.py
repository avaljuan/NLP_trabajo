...
def scrape_article(url: str) -> dict[str, str]:
    ...
    # Título: usamos og:title (metadato estable que usan redes sociales).
    # Si por lo que sea no estuviera, caemos al h2.art__title o a un h1.
    og_title = soup.find("meta", property="og:title")
    if og_title and og_title.get("content"):
        title = og_title["content"].strip()
    else:
        title_tag = soup.select_one("article h2.art__title") or soup.find("h1")
        title = title_tag.get_text(strip=True) if title_tag else ""

    # Fecha de publicación real del artículo (no la del día del scrapeo).
    meta_fecha = soup.find("meta", property="article:published_time")
    fecha_publicacion = meta_fecha["content"][:10] if meta_fecha and meta_fecha.get("content") else ""
