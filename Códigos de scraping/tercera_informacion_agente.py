...
DEFAULT_OUTPUT_CSV = "data_tercerainformacion_opinion.csv"
...
def extraer_urls_listado(soup: BeautifulSoup, page_url: str) -> list[dict[str, str]]:
    articulos = []
    vistos = set()
...
# ---------------------------------------------------------------------------
# Generación de listados y guardado
# ---------------------------------------------------------------------------
...
def guardar_csv(
    ...
    if not datos:
        return pd.DataFrame(columns=["periodico", "titulo", "texto", "url"])
...
def scrape_tercerainformacion(
    ...
    Args:
        ...
        output_csv: ruta del CSV de salida. Si es None, no guarda CSV.
    ...
    for current_url in urls_listado:
        ...
        if output_csv is not None:
            guardar_csv(datos, output_csv=output_csv)
    ...
    if output_csv is not None:
        df = guardar_csv(datos, output_csv=output_csv)
    else:
        df = pd.DataFrame(datos)
        if not df.empty:
            df = df.drop_duplicates(subset=["url"])
